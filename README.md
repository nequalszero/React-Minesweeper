...
- Run `npm start` to start the dev server; this will by default open the app on `localhost:3000` and watch for changes.

### General structure of this repo:
- `src/logic` - contains the logic of the `MineSweeper` game.  The entry point is the Game class, which accepts a difficulty, initializes a `GameSettings` instance, which is in turn used to instantiate a `Board` instance.  The `Board` class instantiates `Tile` instances.
<br><br>
- `src/components` - contains a `MineSweeper` React class component, which is rendered by the `App` component, and a `mine_sweeper` folder, that contains React functional sub-components of the mine sweeper game. The `MineSweeper` component instantiates an instance of the `Game` class, and defers all updating
logic to that class, updating its state with a `"raw"` copy of the board after each change to the board.
